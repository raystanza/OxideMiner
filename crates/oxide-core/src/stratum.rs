...
impl PoolJob {
    pub fn parse(v: Value) -> Result<Self> {
        let mut job: PoolJob = serde_json::from_value(v)?;
        job.target_u32 = if job.target.len() <= 8 {
            if let Ok(mut b) = hex::decode(&job.target) {
                if b.len() > 4 {
                    b.truncate(4);
                }
                while b.len() < 4 {
...
impl StratumClient {
                ...
                Ok(v) => {
                    if let Some(obj) = v.get("result") {
                        ...
                        if let Some(job_val) = obj.get("job") {
                            if let Ok(job) = PoolJob::parse(job_val.clone()) {
                    ...
                    if v.get("method").and_then(Value::as_str) == Some("job") {
                        if let Some(params) = v.get("params") {
                            if let Ok(job) = PoolJob::parse(params.clone()) {
    ...
    pub async fn next_job(&mut self) -> Result<PoolJob> {
        loop {
            ...
            if v.get("method").and_then(Value::as_str) == Some("job") {
                if let Some(params) = v.get("params") {
                    let job = PoolJob::parse(params.clone()).context("parse job params")?;
                    return Ok(job);
                }
            }
        }
