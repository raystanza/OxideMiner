...
- Threshold source: `perf_baselines/ci/manifest.txt`
...
- That step validates parent-facing lifecycle wiring and emitted report shape.
- It is not part of the perf-threshold decision, and it does not replace host-authority benchmarking.

Why these three rows:

- `light_interp` keeps the lowest-risk supported fallback under regression watch.
- `light_jit` keeps the conservative JIT fallback under regression watch.
...
The exact runtime flags for each gated scenario are encoded in `perf_baselines/ci/manifest.txt`.
...
- thresholds after the refresh:
  ...
  - rationale: the same-`HEAD` local gate rerun on this Intel `6/58` host landed at `+2.057%` for `light_interp`, `+4.588%` for `light_jit`, and `+17.164%` for `fast_jit_fastregs`, so only the Fast supported-path row needed extra headroom
- command shape:
  - Light rows: `./target/.../perf_harness --mode light --iters 10 --warmup 2 --format csv`
  - Fast row: `OXIDE_RANDOMX_FAST_BENCH=1 ./target/.../perf_harness --mode fast --jit on --jit-fast-regs on --iters 10 --warmup 2 --format csv`
- v9 alignment note:
  - no fixture replacement was required for the prompt-05 / prompt-07 supported-path refresh
  - the mandatory CI scenario set and supported perf contract remained `light_interp`, `light_jit`, and `fast_jit_fastregs`

## Refresh flow

1. Prefer running the `Supported-Path Perf Gate Capture` workflow with `workflow_dispatch` on the commit you want to bless.
2. Download the `perf-gate-artifacts` artifact, then inspect `compare/*.txt` and the matching `candidate/*.csv` files.
...
When `scripts/ci/run_ci_perf_gate.sh` is invoked from `bash` on Windows, it
resolves the built `.exe` tool names automatically before comparing fixtures.
